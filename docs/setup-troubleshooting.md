...
| MCP tools do not appear in chat | The servers are preset in `.vscode/mcp.json` but never start on their own. `Ctrl/Cmd+Shift+P` → **MCP: List Servers** → the server → **Start Server**. Check the status bar does not say **Restricted Mode** (if it does, trust the folder). Reload the VS Code window and set the agent picker to **Agent**. Ran a bare `init-agents`? Run `npm run agents` instead — it repairs `.vscode/mcp.json`. |
