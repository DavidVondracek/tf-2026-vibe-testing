...
7. **AI models — nothing to do yet.** The Vercel AI Gateway key is handed out at the workshop, at 09:00. On the day: `Ctrl/Cmd+Shift+P` → **Vercel AI Gateway: Manage Authentication** → paste the key (starts with `vck_`), start a new chat (`Ctrl/Cmd+Alt+I`), check the model picker shows **DeepSeek V4.1 Flash**, and send "hi". Until then, Copilot's own free models work for trying things out.
