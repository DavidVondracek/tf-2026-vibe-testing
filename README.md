...
7. **Add the AI key:** after you submit the form (step 2) you get a document with two Vercel AI Gateway keys. In VS Code: `Ctrl/Cmd+Shift+P` → **Vercel AI Gateway: Manage Authentication** → paste **key 1**. Then open a new chat, pick **Agent**, type `gpt-6-luna` in the model picker, choose **GPT-6 Luna · Vercel AI Gateway** and send `hi`. Keep the document: key 2 is the backup.
